...
func TestGetCurrentSets(t *testing.T) {
	...
	if len(currentSets) != 4 {
		t.Error("It should add 4 sets")
	}
	if len(currentSets["set1"]) != 3 {
		t.Error("It should have only 3 featureFlags")
	}
	if _, ok := currentSets["set1"]["split1"]; !ok {
		t.Error("split1 should be present in set1")
	}
	if _, ok := currentSets["set1"]["split2"]; !ok {
		t.Error("split2 should be present in set1")
	}
	if _, ok := currentSets["set1"]["split3"]; !ok {
		t.Error("split3 should be present in set1")
	}

	if len(currentSets["set2"]) != 1 {
		t.Error("It should have only 1 featureFlags")
	}
	if _, ok := currentSets["set2"]["split2"]; !ok {
		t.Error("split2 should be present in set2")
	}

	if len(currentSets["set3"]) != 1 {
		t.Error("It should have only 1 featureFlags")
	}
	if _, ok := currentSets["set3"]["split2"]; !ok {
		t.Error("split2 should be present in set3")
	}

	if len(currentSets["set4"]) != 1 {
		t.Error("It should have only 1 featureFlags")
	}
	if _, ok := currentSets["set4"]["split3"]; !ok {
...
func TestCalculateSets(t *testing.T) {
	...
	currentSets["set1"] = make(map[string]struct{})
	currentSets["set1"]["split1"] = struct{}{}
	currentSets["set1"]["split2"] = struct{}{}
	currentSets["set1"]["split3"] = struct{}{}
	currentSets["set2"] = make(map[string]struct{})
	currentSets["set2"]["split2"] = struct{}{}
	currentSets["set3"] = make(map[string]struct{})
	currentSets["set3"]["split2"] = struct{}{}
	currentSets["set4"] = make(map[string]struct{})
	currentSets["set4"]["split3"] = struct{}{}
	...
	if _, ok := toAdd["set1"]["split5"]; !ok {
		t.Error("split5 should be present in set1")
	}
	if len(toAdd["set4"]) != 1 {
		t.Error("It should have only 1 featureFlags")
	}
	if _, ok := toAdd["set4"]["split1"]; !ok {
