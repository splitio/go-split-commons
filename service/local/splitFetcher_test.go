...
import (
	...
	"github.com/splitio/go-split-commons/v5/service/local/mocks"
...
func TestLocalSplitFetcher(t *testing.T) {
	...
	res, err := fetcher.Fetch(service.MakeSplitFetchOptions(nil).WithChangeNumber(-1))
	if err != nil {
	...
	res, err = fetcher.Fetch(service.MakeSplitFetchOptions(nil).WithChangeNumber(0))
	if err != nil {
		t.Error("fetching should not fail. Got: ", err)
	}
...
func TestLocalSplitFetcherJsonTest1(t *testing.T) {
	...
	_, err = fetcher.Fetch(service.MakeSplitFetchOptions(nil).WithChangeNumber(-1))
...
func TestFetchSomeSplits(t *testing.T) {
	...
	splitChange, _ := mockedFetchers.Fetch(service.MakeSplitFetchOptions(nil).WithChangeNumber(-1))
	...
	splitChange, _ = mockedFetchers.Fetch(service.MakeSplitFetchOptions(nil).WithChangeNumber(2323))
	if splitChange.Since != 2323 || splitChange.Till != 2323 {
		t.Error("Wrong since/till. Got: ", splitChange.Since, splitChange.Till)
	}
	if len(splitChange.Splits) != 1 {
